...
class Ultima_RAGDatabase:
    ...
    def page_out_messages(self, message_ids: List[str]):
        """Move messages to PAGED state to free up LLM context."""
        table = self.conn.open_table("messages")
        # In LanceDB, updates are often done by overwriting or semi-joins. 
        # For simplicity in this spec, we simulate the logic.
    ...
    def search_messages(self, query_vector: List[float], limit: int = 5, conversation_id: Optional[str] = None) -> List[Dict]:
        """Perform vector search across all messages (The MemGPT 'Recall' logic)."""
        table = self.conn.open_table("messages")
    ...
    def search_knowledge(self, query_vector: List[float], query_text: str, project_id: Optional[str] = None, 
        ...
        """Search the knowledge base using Hybrid SOTA (Vector + BM25), with optional file/chat filtering."""
        table = self.conn.open_table("knowledge_base")
